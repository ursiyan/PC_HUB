...
$data = $pdo->query("SELECT `name`,`diagonal`,`resolution`,`martix_type`,`frequency`,`price`,`rate` FROM `monitors` WHERE  `id` ='" . $proc1 . "';")->fetchAll();
$data2 = $pdo->query("SELECT `name`,`diagonal`,`resolution`,`martix_type`,`frequency`,`price`,`rate` FROM `monitors` WHERE  `id` ='" . $proc2 . "';")->fetchAll();
$data3 = $pdo->query("SELECT `name`, `rate` FROM `monitors` ORDER BY `rate` DESC;")->fetchAll();
...
<br><br><br>
<center>
	<form method='POST' action='monitors.php'>
	<table>	
    <h1 style="color:white; font-size:200%">Сравнение мониторов</h1><br><br>
	...
	<th><select class="form-control input-xs"   id="subject-select" name="proc1">
			<option selected disabled hidden> Choose here </option>
				...
				$result = mysqli_query($connection, "SELECT * FROM `monitors`");
...
<?php 
	if ($proc1!==null && $proc2!==null){ // Проверка на ненулевое значение переменной с ID процессоров
				...
				;} if ($data[0][3]<>$data2[0][3]){
				  ...
				  ;} else { 
					...
					;} if ($data[0][4]<>$data2[0][4]){
						echo "
						<tr>
						  <td class='red'>Частота обновления</td>
		...
		<?php
						...
						<table class='top'>
						<h1 style='color:white; margin-top:5%; font-size:300%'>Топ мониторов</h1><br>
