...
$data3 = $pdo->query("SELECT `name`, `rate` FROM `memory_storage` ORDER BY `rate` DESC;")->fetchAll();
...
<?php 
	if ($proc1!==null && $proc2!==null){ // Проверка на ненулевое значение переменной с ID процессоров
			...
			;} if ($data[0][2]<>$data2[0][2]){
			  ...
			  ;} else { 
				...
				;} if ($data[0][3]<>$data2[0][3]){
					echo "
					<tr>
					  <td class='red'>Скорость чтения</td>
				  ...
				  ;} else { 
					...
					;} if ($data[0][4]<>$data2[0][4]){
						echo "
						<tr>
						  <td class='red'>Скорость записи</td>
		...
		<?php
						...
						<table class='top'>
						<h1 style='color:white; margin-top:5%; font-size:300%'>Топ хранилищ</h1><br>
